...
export function WhiteBoard() {
  ...
  const updateDbElement = useMutation(api.whiteboards.updateElementsBySessionId);
  const getDbElement = useQuery(api.whiteboards.getBySession, { sessionId: id as any });
  ...
  useEffect(() => {
    (async () => {
      setElements(JSON.parse(getDbElement[0].elements), true);
    })()
  }, [getDbElement])


  const saveElementsToServer = (newElements: ElementType[]) => {
    if (id) {
      updateDbElement({
        sessionId: id as any,
